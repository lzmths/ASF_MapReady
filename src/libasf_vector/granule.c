...
void gran_init(granule_type_t *gran)
{
  ...
  strcpy(gran->start_date, MAGIC_UNSET_STRING);
  strcpy(gran->end_date, MAGIC_UNSET_STRING);
...
static int read_gran_line(char *header, int n, char *line, granule_type_t *gran)
{
  ...
  for (ii=0; ii<n; ii++) {
    ...
    else if (strcmp(test, "Start Date") == 0)
      strcpy(gran->start_date, get_str(line, ii));
    else if (strcmp(test, "End Date") == 0)
      strcpy(gran->end_date, get_str(line, ii));
			 ...
			 double *lat, double *lon)
{
  ...
  for (ii=0; ii<nCols; ii++) {
    ...
    else if (strcmp(dbf[ii].header, "Start Date") == 0 && dbf[ii].visible) {
      DBFWriteStringAttribute(dbase, n, field, gran->start_date);
      field++;
    }
    else if (strcmp(dbf[ii].header, "End Date") == 0 && dbf[ii].visible) {
      DBFWriteStringAttribute(dbase, n, field, gran->end_date);
      field++;
    }
