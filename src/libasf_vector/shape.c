...
void shape_init(char *inFile, format_type_t format)
{
  ...
  if (format == RGPS) {
    ...
    if (DBFAddField(dbase, "GRANULE", FTString, 60, 0) == -1)
    ...
    if (DBFAddField(dbase, "ST_DATE", FTString, 25, 0) == -1)
      asfPrintError("Could not add 'ST_DATE' field to database file\n");
    if (DBFAddField(dbase, "END_DATE", FTString, 25, 0) == -1)
      asfPrintError("Could not add 'END_DATE' field to database file\n");
    ...
    if (DBFAddField(dbase, "INSAR", FTInteger, 1, 0) == -1)
